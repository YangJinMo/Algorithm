...
/**
 ...
 - 원문: https://www.acmicpc.net/problem/8958
 */
...
class Problem_8958 {
  ...
  func solution_2() {
    var a=0,b=0;for _ in 1...Int(readLine()!)!{readLine()!.map{if $0=="O"{a+=1}else{a=0};b+=a};print(b);a=0;b=0}
  }
  func solution_3() {
    (1...Int(readLine()!)!).map{_ in print(readLine()!.split{$0=="X"}.map{($0.count+1)*$0.count/2}.reduce(0,+))}
  }
  /// https://www.acmicpc.net/short/status/8958/74/1
  /// 1등
  func solution_zzimss() {
    for _ in 1...Int(readLine()!)!{print(readLine()!.split{$0=="X"}.map{($0.count+1)*$0.count/2}.reduce(0,+))}
  }
}
